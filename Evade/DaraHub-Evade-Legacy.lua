...
local v1 = game.Players.LocalPlayer.PlayerScripts.CameraCFrame.Value:VectorToWorldSpace(Vector3.new(self.f.b, 0, self.f.a))
...
local path = workspace.Game.Players:FindFirstChild(player.Name)
if path then
...
charAddedConnection = Player.CharacterAdded:Connect(handleCharacterAdded)
...
player = game:GetService("Players").LocalPlayer humanoidSpeed = 0 humanoidConnection = nil originalWalkSpeed = nil function updateHumanoidSpeed() if humanoidConnection then humanoidConnection:Disconnect() humanoidConnection = nil end character = player.Character if not character then return end humanoid = character:FindFirstChild("Humanoid") if not humanoid then return end if originalWalkSpeed == nil then originalWalkSpeed = humanoid.WalkSpeed end if humanoidSpeed == 0 then humanoid.WalkSpeed = originalWalkSpeed else humanoid.WalkSpeed = originalWalkSpeed + humanoidSpeed humanoidConnection = humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function() if humanoid.WalkSpeed ~= originalWalkSpeed + humanoidSpeed then humanoid.WalkSpeed = originalWalkSpeed + humanoidSpeed end end) end end function onCharacterAdded(character) task.wait(0.5) originalWalkSpeed = nil updateHumanoidSpeed() end if player.Character then updateHumanoidSpeed() end player.CharacterAdded:Connect(onCharacterAdded) Tabs.Player:Space() Tabs.Player:Input({ Title = "Humanoid.WalkSpeed", Desc = "This can be use to bounce too", Value = "0", Callback = function(value) newSpeed = tonumber(value) or 0 humanoidSpeed = newSpeed updateHumanoidSpeed() end }) Tabs.Player:Space()
...
if cachedController and cachedController.Character == lp.Character then
...
if player.Character and AutoSelfRevive then
local char = player.Character
